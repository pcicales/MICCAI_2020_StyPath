...
for i in range(0, 20000):
    img_name_list_temp = img_name_list.copy()
    style = random.choice(img_name_list_temp)
    img_name_list_temp.remove(style)
    content = random.choice(img_name_list_temp)
    ...
    def imsave(tensor, content, style):
        image = tensor.cpu().clone()  # we clone the tensor to not do changes on it
        image = image.squeeze(0)  # remove the fake batch dimension
        image = unloader(image)
        image.save('<save_dir>' +
                    content.split('/')[-1].split('.')[0] + '|||' + style.split('/')[-1].split('.')[0] + '.jpg', 'JPEG')

    imsave(output, content, style)
